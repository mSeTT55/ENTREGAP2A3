...
export async function createTables () {
    ...
    sql_createtable_usuarios = `CREATE TABLE IF NOT EXISTS [usuarios] ([idusuario] INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL UNIQUE,[nome_completo] TEXT NOT NULL,[email] TEXT NOT NULL UNIQUE,[senha] TEXT NOT NULL,[confirm_senha] TEXT NOT NULL, FOREIGN KEY([idusuario]) REFERENCES [situacao_serie]([usuario_idusuario]) ON DELETE CASCADE ON UPDATE CASCADE)`;
    sql_createtable_series = `CREATE TABLE IF NOT EXISTS [series] ([idseries] INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL UNIQUE,[plataforma_idplataforma] INTEGER NOT NULL,[nome] TEXT NOT NULL,[ano] INTEGER NOT NULL,[genero] TEXT NOT NULL,[imagem_serie] TEXT NOT NULL,[sinopse] TEXT NOT NULL,[temporada] INTEGER NOT NULL,FOREIGN KEY([idseries]) REFERENCES [situacao_serie]([series_idseries]) ON DELETE CASCADE ON UPDATE CASCADE, FOREIGN KEY([plataforma_idplataforma]) REFERENCES [situacao_serie]([series_plataforma_idplataforma]))`;
    sql_createtable_situacao_serie = `CREATE TABLE IF NOT EXISTS [situacao_serie] ([idsituacao_serie] INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL UNIQUE,[desejo_assistir] TEXT NOT NULL,[assistido] TEXT NOT NULL, [recomendado] TEXT, [series_idseries] INTEGER NOT NULL,[series_plataforma_idplataforma] INTEGER NOT NULL,[usuario_idusuario] INTEGER NOT NULL)`;
